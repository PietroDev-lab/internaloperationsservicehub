...
export class RequestsService {
  ...
  private seedDemoQueue() {
    const seedRequests: Array<{ employee_id: string; type: string; description: string; status: RequestStatus; createdAtOffsetHours: number; }> = [
      {
        employee_id: 'EMP-101',
        type: 'Official Document',
        description: 'Need a duplicate contract for my onboarding package and a signature confirmation.',
        status: 'Received',
      ...
      },
      {
        employee_id: 'EMP-208',
        type: 'Time Off',
        description: 'Requesting two days of annual leave for a family event next week.',
        status: 'In Progress',
      ...
      },
      {
        ...
        description: 'Need a replacement monitor and docking station for my temporary workspace setup.',
        status: 'Resolved',
      ...
      },
      {
        employee_id: 'EMP-440',
        type: 'Official Document',
        description: 'Requesting a notarized travel approval letter before my upcoming trip.',
        status: 'Delayed',
    ...
    for (const seed of seedRequests) {
      const createdAt = new Date(Date.now() - seed.createdAtOffsetHours * 60 * 60 * 1000);
